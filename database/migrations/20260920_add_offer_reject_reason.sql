-- Teklifler tablosuna Kabul Edilmeme (Ret) nedeni, detayı ve tarihi alanlarının eklenmesi
-- Tarih: 2026-09-20

-- 1. offers tablosuna kolonları ekle (IF NOT EXISTS)
ALTER TABLE `offers` 
    ADD COLUMN IF NOT EXISTS `reject_reason` VARCHAR(100) NULL DEFAULT NULL AFTER `statu`,
    ADD COLUMN IF NOT EXISTS `reject_detail` TEXT NULL DEFAULT NULL AFTER `reject_reason`,
    ADD COLUMN IF NOT EXISTS `reject_date` VARCHAR(20) NULL DEFAULT NULL AFTER `reject_detail`;

-- 2. view_offers görünümünü (VIEW) güncelle
