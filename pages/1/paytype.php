...
$statuCode = 3; // Tahsilat Türü
$pageSlug = "paytype";
$pageTitle = "Tahsilat Türü Tanımlama";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Lütfen tahsilat türü adını giriniz.']);
            exit;
        }
    } else {
        if ($type === "new" || $id === 0) {
            try {
                ...
                $ekle->execute([$title, $regdate, $statuCode, $creator, "Tahsilat Türü Tanımı"]);
                $lastId = $ac->lastInsertId();

                if (function_exists('audit_log')) {
                    audit_log("create", $pageSlug, "Yeni tahsilat türü eklendi: " . $title, "units", (string)$lastId);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Tahsilat türü başarıyla eklendi.', 'id' => $lastId]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Paytype insert error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            try {
                ...
                if (function_exists('audit_log')) {
                    audit_log("update", $pageSlug, "Tahsilat türü güncellendi: " . $title, "units", (string)$id);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Tahsilat türü başarıyla güncellendi.', 'id' => $id]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Paytype update error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json', true, 500);
...
if (!empty($_GET["id"]) && @$_GET["mode"] === "delete" && @$_GET["code"] === "04md177") {
    ...
    if (function_exists('audit_log')) {
        audit_log("delete", $pageSlug, "Tahsilat türü silindi. ID: " . $delId, "units", (string)$delId);
    }
}
...
try {
    ...
    error_log("Paytype stats error: " . $e->getMessage());
}
?>
    ...
    <div class="premium-header-card mb-4">
        <div class="header-content">
            <div class="header-left">
                <div class="header-icon" style="background: linear-gradient(135deg, #d97706 0%, #f59e0b 100%); color: #ffffff; box-shadow: 0 4px 10px rgba(245, 158, 11, 0.25);">
                ...
                <div class="header-title">
                    ...
                    <div class="header-stat-pills d-flex align-items-center flex-wrap" style="gap: 8px; margin-top: 6px;">
                        <span class="header-pill header-pill-total" style="background: #fffbeb; border-color: #fde68a; color: #b45309;" title="Toplam Kayıt">
                            <i class="fa fa-list-ul mr-1"></i> Toplam: <strong style="color: #92400e;"><?php echo $totalCount; ?></strong>
                        </span>
                        <?php if ($lastAddedTitle !== "-"): ?>
                        <span class="header-pill header-pill-recent" title="Son Eklenen Tahsilat Türü">
            ...
            <div class="header-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <a href="index.php?p=services" class="btn-header btn-header-secondary" title="Servisler Listesine Git">
                    <i class="fa fa-cogs mr-1 text-warning"></i> Servisler
                </a>
                <button type="button" class="btn-header btn-header-save" style="background: linear-gradient(135deg, #d97706 0%, #b45309 100%); box-shadow: 0 2px 6px rgba(217, 119, 6, 0.35);" id="btnOpenNewModal">
                    <i class="fa fa-plus-circle mr-1"></i> Yeni Tahsilat Türü
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="form-card mb-4 animate-fade-in" style="padding: 0; overflow: hidden; border-radius: 14px;">
        <div class="form-card-header d-flex justify-content-between align-items-center flex-wrap" style="padding: 16px 20px; border-bottom: 1px solid #f1f5f9; gap: 15px;">
            <div class="d-flex align-items-center header-left-inner">
                <div class="card-icon mr-3" style="width: 40px; height: 40px; border-radius: 10px; font-size: 16px; display: flex; align-items: center; justify-content: center; background: rgba(245, 158, 11, 0.1); color: #d97706;">
                    <i class="fa fa-money"></i>
                </div>
                <div>
                    <h5 class="mb-0" style="font-size: 16px; font-weight: 700;">Tanımlı Tahsilat ve Ödeme Türleri</h5>
                    <p class="mb-0 text-muted" style="font-size: 12.5px;">Servis, bakım ve teklif işlemlerinde geçerli ödeme ve tahsilat tipleri (Sağ tık menüsü desteklenir)</p>
                </div>
            </div>
        </div>

        <div class="table-responsive" style="padding: 0; margin: 0;">
            <table id="paytypeTable" class="data-table select-row table-bordered table-hover" style="width: 100%; margin: 0 !important;">
                <thead>
                    <tr>
                        <th class="text-center no-filter" style="width: 60px; max-width: 60px;">#Sıra</th>
                        <th style="min-width: 240px;">Tahsilat Türü</th>
                ...
                <tbody>
                    ...
                    try {
                            ...
                            <tr id="row-unit-<?php echo $as["id"]; ?>" data-id="<?php echo $as["id"]; ?>" data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="definition-tag-dot mr-2" style="background: #d97706;"></div>
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="user-avatar-mini mr-2" style="width: 28px; height: 28px; border-radius: 50%; background: #fef3c7; color: #b45309; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">
                                ...
                                <td class="text-center text-muted" style="vertical-align: middle; font-size: 12.5px;">
                                    <i class="fa fa-calendar-o mr-1 text-warning"></i> <?php echo htmlspecialchars($as["regdate"] ?? '-', ENT_QUOTES, 'UTF-8'); ?>
                                </td>
                        ...
                        echo '<tr><td colspan="6" class="text-center text-danger py-4"><i class="fa fa-exclamation-triangle mr-2"></i> Tahsilat türleri yüklenirken bir hata oluştu: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . '</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
...
<!-- Tahsilat Türü Ekleme / Düzenleme Modalı -->
        ...
        <div class="modal-content premium-modal-content">
            <form method="POST" id="myForm" onsubmit="return false;">
                ...
                <div class="modal-header premium-modal-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <div class="modal-header-icon mr-3" style="background: linear-gradient(135deg, #d97706 0%, #f59e0b 100%);">
                            <i class="fa fa-credit-card" id="modalHeaderIcon"></i>
                        </div>
                        <div>
                            <h5 class="modal-title font-weight-bold mb-0" id="exampleModalLongTitle">Yeni Tahsilat Türü</h5>
                            <small class="text-muted" id="modalSubtitle">Sistemde kullanılacak yeni bir tahsilat türü tanımlayın</small>
                        </div>
                    </div>
                ...
                <div class="modal-body p-4">
                    <div class="form-group mb-3">
                        <label for="title" class="font-weight-600 text-dark mb-1">
                            Tahsilat Türü Adı <span class="text-danger">*</span>
                        </label>
                        <div class="input-group input-group-modern">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><i class="fa fa-credit-card text-muted"></i></span>
                            </div>
                            <input required type="text" class="form-control form-control-modern" name="title" id="title" placeholder="Örn: Nakit, Kredi Kartı" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div class="suggestion-box p-3 rounded" style="background: #f8fafc; border: 1px dashed #cbd5e1;">
                        ...
                        <div class="suggestion-tags d-flex flex-wrap" style="gap: 8px;">
                            <span class="badge badge-pill quick-tag" role="button" data-val="Nakit"><i class="fa fa-tag mr-1 text-muted"></i> Nakit</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Kredi Kartı"><i class="fa fa-tag mr-1 text-muted"></i> Kredi Kartı</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Havale / EFT"><i class="fa fa-tag mr-1 text-muted"></i> Havale / EFT</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Çek"><i class="fa fa-tag mr-1 text-muted"></i> Çek</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Mail Order"><i class="fa fa-tag mr-1 text-muted"></i> Mail Order</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="30 Gün Vadeli"><i class="fa fa-tag mr-1 text-muted"></i> 30 Gün Vadeli</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Cari Hesap"><i class="fa fa-tag mr-1 text-muted"></i> Cari Hesap</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer premium-modal-footer d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" id="submitButtonByAjax" class="btn btn-primary btn-modern-save" style="background: linear-gradient(135deg, #d97706 0%, #b45309 100%);">
...
$(document).ready(function() {
    var pageSlug = "paytype";
    var entityLabel = "Tahsilat Türü";
    ...
    // ==========================================
    $(document).on("contextmenu", "#paytypeTable tbody tr", function(e) {
        if ($(this).find("td").length <= 1) return;

        e.preventDefault();

        var $tr = $(this);
        $("#paytypeTable tbody tr").removeClass("context-menu-active");
        ...
        var menuHtml = '<div class="cm-header"><i class="fa fa-credit-card mr-1 text-warning"></i> ' + $("<div>").text(title || entityLabel).html() + '</div>';
