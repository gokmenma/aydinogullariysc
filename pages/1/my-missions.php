...
if (@$_GET["mode"] === "update" && isset($_GET["mid"])) {
    ...
    // Kullanıcının bu göreve atanmış olduğunu veya yetkisi olduğunu doğrula
    $mission = $missionModel->getMissionById($mid);
    if ($mission) {
        $authors = array_filter(explode('|', $mission['authors'] ?? ''));
        if (in_array((string)$currentUserId, $authors) || in_array($currentUserId, $authors) || permtrue("allmisview") || $mission['creativer'] == $currentUserId) {
...
$myMissions = $missionModel->getMyMissions($currentUserId);
$stats = $missionModel->getMissionStats($myMissions);
...
<script>
    (function() {
        try {
            if (localStorage.getItem('aydinogullari_kpi_missions_collapsed') === 'true') {
                document.documentElement.classList.add('kpi-missions-collapsed-early');
            }
        } catch(e) {}
    })();
</script>
...
<div class="mission-module-wrapper">
    ...
    <div class="premium-header-card animate-fade-in mb-3">
        <div class="header-content">
            <div class="header-left">
                <div class="header-icon mission-header-icon">
    ...
    <!-- Compact KPI Summary Grid (Aligned Perfectly With Top & Bottom Cards) -->
    <div id="kpiSummarySection" class="mission-kpi-grid mb-3 animate-fade-in kpi-summary-collapse">
        ...
        <div class="crm-kpi-card">
            <div class="crm-kpi-header">
                <div>
                    <span class="crm-kpi-label">Bekleyen Görev</span>
    ...
    <?php if (@$_GET["update"] === "true"): ?>
        <div class="alert alert-success alert-dismissible fade show animate-fade-in mb-3" role="alert" style="border-radius: 10px;">
            <i class="fa fa-check-circle mr-2"></i> <strong>Tebrikler!</strong> Görev başarıyla tamamlandı olarak işaretlendi.
    ...
    <div class="form-card mb-4 animate-fade-in mission-table-card">
        <div class="form-card-header mission-table-header">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h5 class="mb-0">Görev Listem</h5>
                    <p class="mb-0">Tarafınıza atanmış görevleri inceleyin, durumlarını yönetin ve tamamlayın.</p>
                </div>
            </div>
            <div class="d-flex align-items-center" style="gap: 8px;">
                <div id="missionSearchContainer" class="mission-search-container"></div>
            ...
            <table id="myMissionsTable" class="data-table select-row table-hover table-bordered premium-table"
                ...
                <thead>
                    <tr>
                        ...
                        <th data-filter-type="text" style="width: 170px;">Görevi Veren</th>
                        ...
                        <th scope="col" class="text-center no-filter" style="width: 140px;">İşlemler</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <tr class="<?php echo $isDone ? 'mission-row-done' : ($isOverdue ? 'mission-row-overdue' : ''); ?>">
                                ...
                                <td class="text-center">
                                    <div class="mission-action-buttons">
                                        ...
                                        <?php if (!$isDone): ?>
                                            <button type="button" 
                                                    class="btn-table-action btn-table-complete" 
                                                    title="Yapıldı Olarak İşaretle" 
                                                    onclick="confirmCompleteMission(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars(addslashes($row['title'])); ?>')">
                                                <i class="fa fa-check"></i> <span>Tamamla</span>
                                            </button>
                                        <?php endif; ?>
                                    </div>
                                </td>
...
<style>
/* Flicker Prevention */
.kpi-missions-collapsed-early #kpiSummarySection {
...
.dark-mode .mission-username {
    color: #f1f5f9;
}

.dark-mode .mission-usertitle,
...
.dark-mode .btn-table-complete {
    background: rgba(5, 150, 105, 0.2);
    border-color: rgba(5, 150, 105, 0.4);
    color: #6ee7b7;
}

.dark-mode .btn-table-complete:hover {
...
<script>
function confirmCompleteMission(missionId, missionTitle) {
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = 'index.php?p=my-missions&mode=update&statu=1&mid=' + missionId;
            }
        });
    } else {
...
$(document).ready(function () {
    ...
    var KPI_STORAGE_KEY = 'aydinogullari_kpi_missions_collapsed';
    ...
    function initMyMissionsTable() {
        $("#myMissionsTable").find("tr.search-input-row").remove();

        var filterEl = $("#myMissionsTable_wrapper .dataTables_filter");
        if (filterEl.length && !$("#missionSearchContainer .dataTables_filter").length) {
            filterEl.appendTo("#missionSearchContainer");
            if (filterEl[0]) filterEl[0].dataset.relocated = 'true';
        }

        if (window.App && window.App.TableFilter) {
            App.TableFilter.attachToTable(document.getElementById("myMissionsTable"));
        }
    }
    ...
    $(document).on('click', '.btn-preview-mission', function (e) {
        ...
        if (data.canComplete) {
            $('#previewModalCompleteBtn')
                .show()
                .off('click')
                .on('click', function() {
                    $('#missionPreviewModal').modal('hide');
                    confirmCompleteMission(data.id, data.title);
                });
        } else {
            $('#previewModalCompleteBtn').hide();
        }
    ...
    initMyMissionsTable();
    setTimeout(initMyMissionsTable, 100);
    setTimeout(initMyMissionsTable, 300);
});
</script>
