    ...
    try {
        ...
        if (!empty($urunAdi) && is_array($urunAdi)) {
            for ($i = 0; $i < count($urunAdi); $i++) {
                $insqItem = $ac->prepare("INSERT INTO purchase_items SET purID = ?, 
																stokKodu = ? ,
																product = ? , 
																amount = ? , 
																unit = ? , 
        ...
        error_log("Satın alma talebi güncelleme hatası: " . $e->getMessage());
        showAlert('danger', "Talep güncellenirken bir hata oluştu: " . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8'));
    }
