...
$statuCode = 5; // Servis Bölgesi
$pageSlug = "service-region";
$pageTitle = "Servis Bölgesi Tanımlama";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Lütfen servis bölgesi adını giriniz.']);
            exit;
        }
    } else {
        if ($type === "new" || $id === 0) {
            try {
                ...
                $ekle->execute([$title, $regdate, $statuCode, $creator, "Servis Bölgesi Tanımı"]);
                $lastId = $ac->lastInsertId();

                if (function_exists('audit_log')) {
                    audit_log("create", $pageSlug, "Yeni servis bölgesi eklendi: " . $title, "units", (string)$lastId);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Servis bölgesi başarıyla eklendi.', 'id' => $lastId]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Service region insert error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            try {
                ...
                if (function_exists('audit_log')) {
                    audit_log("update", $pageSlug, "Servis bölgesi güncellendi: " . $title, "units", (string)$id);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Servis bölgesi başarıyla güncellendi.', 'id' => $id]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Service region update error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json', true, 500);
...
if (!empty($_GET["id"]) && @$_GET["mode"] === "delete" && @$_GET["code"] === "04md177") {
    ...
    if (function_exists('audit_log')) {
        audit_log("delete", $pageSlug, "Servis bölgesi silindi. ID: " . $delId, "units", (string)$delId);
    }
}
...
try {
    ...
    error_log("Service region stats error: " . $e->getMessage());
}
?>
    ...
    <div class="premium-header-card mb-4">
        <div class="header-content">
            <div class="header-left">
                <div class="header-icon" style="background: linear-gradient(135deg, #059669 0%, #10b981 100%); color: #ffffff; box-shadow: 0 4px 10px rgba(16, 185, 129, 0.25);">
                ...
                <div class="header-title">
                    ...
                    <div class="header-stat-pills d-flex align-items-center flex-wrap" style="gap: 8px; margin-top: 6px;">
                        <span class="header-pill header-pill-total" style="background: #ecfdf5; border-color: #a7f3d0; color: #047857;" title="Toplam Kayıt">
                            <i class="fa fa-list-ul mr-1"></i> Toplam: <strong style="color: #065f46;"><?php echo $totalCount; ?></strong>
                        </span>
                        <?php if ($lastAddedTitle !== "-"): ?>
                        <span class="header-pill header-pill-recent" title="Son Eklenen Bölge">
            ...
            <div class="header-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <a href="index.php?p=customers" class="btn-header btn-header-secondary" title="Müşteriler Listesine Git">
                    <i class="fa fa-building-o mr-1 text-success"></i> Müşteriler
                </a>
                <button type="button" class="btn-header btn-header-save" style="background: linear-gradient(135deg, #059669 0%, #047857 100%); box-shadow: 0 2px 6px rgba(5, 150, 105, 0.35);" id="btnOpenNewModal">
                    <i class="fa fa-plus-circle mr-1"></i> Yeni Servis Bölgesi
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="form-card mb-4 animate-fade-in" style="padding: 0; overflow: hidden; border-radius: 14px;">
        <div class="form-card-header d-flex justify-content-between align-items-center flex-wrap" style="padding: 16px 20px; border-bottom: 1px solid #f1f5f9; gap: 15px;">
            <div class="d-flex align-items-center header-left-inner">
                <div class="card-icon mr-3" style="width: 40px; height: 40px; border-radius: 10px; font-size: 16px; display: flex; align-items: center; justify-content: center; background: rgba(16, 185, 129, 0.1); color: #059669;">
                    <i class="fa fa-globe"></i>
                </div>
                <div>
                    <h5 class="mb-0" style="font-size: 16px; font-weight: 700;">Tanımlı Servis Bölgeleri</h5>
                    <p class="mb-0 text-muted" style="font-size: 12.5px;">Müşteri ve servis operasyonlarında kullanılan coğrafi bölge tanımları (Sağ tık menüsü desteklenir)</p>
                </div>
            </div>
        </div>

        <div class="table-responsive" style="padding: 0; margin: 0;">
            <table id="serviceRegionTable" class="data-table select-row table-bordered table-hover" style="width: 100%; margin: 0 !important;">
                <thead>
                    <tr>
                        <th class="text-center no-filter" style="width: 60px; max-width: 60px;">#Sıra</th>
                        <th style="min-width: 240px;">Servis Bölgesi</th>
                ...
                <tbody>
                    ...
                    try {
                            ...
                            <tr id="row-unit-<?php echo $as["id"]; ?>" data-id="<?php echo $as["id"]; ?>" data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="definition-tag-dot mr-2" style="background: #059669;"></div>
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="user-avatar-mini mr-2" style="width: 28px; height: 28px; border-radius: 50%; background: #d1fae5; color: #047857; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">
                                ...
                                <td class="text-center text-muted" style="vertical-align: middle; font-size: 12.5px;">
                                    <i class="fa fa-calendar-o mr-1 text-success"></i> <?php echo htmlspecialchars($as["regdate"] ?? '-', ENT_QUOTES, 'UTF-8'); ?>
                                </td>
                                ...
                                <td class="text-center" style="vertical-align: middle; white-space: nowrap;">
                                    <div class="btn-group btn-group-sm" role="group">
                                        <?php if ($isAdmin || permtrue("customeredit") || permtrue("serviceEdit") || $userPerm == 1) { ?>
                                        ...
                                        <button type="button" class="btn btn-outline-danger btn-delete-unit" 
                                            ...
                                            data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                            title="Sil" 
                                            onClick="deleteRecord('\'<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>\' servis bölgesini silmek istediğinize emin misiniz?','<?php echo $as["id"]; ?>','service-region','units')">
                        ...
                        echo '<tr><td colspan="6" class="text-center text-danger py-4"><i class="fa fa-exclamation-triangle mr-2"></i> Servis bölgeleri yüklenirken bir hata oluştu: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . '</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
...
<!-- Servis Bölgesi Ekleme / Düzenleme Modalı -->
        ...
        <div class="modal-content premium-modal-content">
            <form method="POST" id="myForm" onsubmit="return false;">
                ...
                <div class="modal-header premium-modal-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <div class="modal-header-icon mr-3" style="background: linear-gradient(135deg, #059669 0%, #10b981 100%);">
                            <i class="fa fa-map-marker" id="modalHeaderIcon"></i>
                        </div>
                        <div>
                            <h5 class="modal-title font-weight-bold mb-0" id="exampleModalLongTitle">Yeni Servis Bölgesi</h5>
                            <small class="text-muted" id="modalSubtitle">Sistemde kullanılacak yeni bir servis bölgesi tanımlayın</small>
                        </div>
                    </div>
                ...
                <div class="modal-body p-4">
                    <div class="form-group mb-3">
                        <label for="title" class="font-weight-600 text-dark mb-1">
                            Servis Bölgesi Adı <span class="text-danger">*</span>
                        </label>
                        <div class="input-group input-group-modern">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><i class="fa fa-map-marker text-muted"></i></span>
                            </div>
                            <input required type="text" class="form-control form-control-modern" name="title" id="title" placeholder="Örn: Avrupa Yakası" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div class="suggestion-box p-3 rounded" style="background: #f8fafc; border: 1px dashed #cbd5e1;">
                        ...
                        <div class="suggestion-tags d-flex flex-wrap" style="gap: 8px;">
                            <span class="badge badge-pill quick-tag" role="button" data-val="Avrupa Yakası"><i class="fa fa-tag mr-1 text-muted"></i> Avrupa Yakası</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Anadolu Yakası"><i class="fa fa-tag mr-1 text-muted"></i> Anadolu Yakası</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Gebze / Kocaeli"><i class="fa fa-tag mr-1 text-muted"></i> Gebze / Kocaeli</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Trakya Bölgesi"><i class="fa fa-tag mr-1 text-muted"></i> Trakya Bölgesi</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Başakşehir & İkitelli"><i class="fa fa-tag mr-1 text-muted"></i> Başakşehir & İkitelli</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Tuzla & Pendik"><i class="fa fa-tag mr-1 text-muted"></i> Tuzla & Pendik</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer premium-modal-footer d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" id="submitButtonByAjax" class="btn btn-primary btn-modern-save" style="background: linear-gradient(135deg, #059669 0%, #047857 100%);">
...
.quick-tag:hover {
    ...
    box-shadow: 0 3px 6px rgba(5, 150, 105, 0.15) !important;
}
.quick-tag:hover i {
...
$(document).ready(function() {
    var pageSlug = "service-region";
    var entityLabel = "Servis Bölgesi";

    // Yeni Modal Açıldığında
    function openNewModal() {
        $("#id").val("0");
        $("#title").val("");
        $("#exampleModalLongTitle").text("Yeni " + entityLabel);
    ...
    // ==========================================
    $(document).on("contextmenu", "#serviceRegionTable tbody tr", function(e) {
        if ($(this).find("td").length <= 1) return;

        e.preventDefault();

        var $tr = $(this);
        $("#serviceRegionTable tbody tr").removeClass("context-menu-active");
        ...
        var menuHtml = '<div class="cm-header"><i class="fa fa-map-marker mr-1 text-success"></i> ' + $("<div>").text(title || entityLabel).html() + '</div>';
