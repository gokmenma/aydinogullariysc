...
$statuCode = 4; // Servis Durumu
$pageSlug = "service-status";
$pageTitle = "Servis Durumu Tanımlama";
...
if (!empty($_GET["id"]) && @$_GET["mode"] === "delete" && @$_GET["code"] === "04md177") {
    ...
    if (function_exists('audit_log')) {
        audit_log("delete", $pageSlug, "Servis durumu silindi. ID: " . $delId, "units", (string)$delId);
    }
}
...
try {
    ...
    error_log("Service status stats error: " . $e->getMessage());
}
?>
    ...
    <div class="premium-header-card mb-4">
        <div class="header-content">
            <div class="header-left">
                <div class="header-icon" style="background: linear-gradient(135deg, #7c3aed 0%, #6366f1 100%); color: #ffffff; box-shadow: 0 4px 10px rgba(124, 58, 237, 0.25);">
                ...
                <div class="header-title">
                    ...
                    <div class="header-stat-pills d-flex align-items-center flex-wrap" style="gap: 8px; margin-top: 6px;">
                        <span class="header-pill header-pill-total" style="background: #f5f3ff; border-color: #ddd6fe; color: #6d28d9;" title="Toplam Kayıt">
                            <i class="fa fa-list-ul mr-1"></i> Toplam: <strong style="color: #5b21b6;"><?php echo $totalCount; ?></strong>
                        </span>
                        <?php if ($lastAddedTitle !== "-"): ?>
                        <span class="header-pill header-pill-recent" title="Son Eklenen Servis Durumu">
            ...
            <div class="header-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <button type="button" class="btn-header btn-header-save" style="background: linear-gradient(135deg, #7c3aed 0%, #6366f1 100%); box-shadow: 0 2px 6px rgba(124, 58, 237, 0.35);" id="btnOpenNewModal">
                    <i class="fa fa-plus-circle mr-1"></i> Yeni Servis Durumu
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="form-card mb-4 animate-fade-in" style="padding: 0; overflow: hidden; border-radius: 14px;">
        <div class="form-card-header d-flex justify-content-between align-items-center flex-wrap" style="padding: 16px 20px; border-bottom: 1px solid #f1f5f9; gap: 15px;">
            <div class="d-flex align-items-center header-left-inner">
                <div class="card-icon mr-3" style="width: 40px; height: 40px; border-radius: 10px; font-size: 16px; display: flex; align-items: center; justify-content: center; background: rgba(124, 58, 237, 0.1); color: #7c3aed;">
            ...
            <table id="serviceStatusTable" class="data-table select-row table-bordered table-hover" style="width: 100%; margin: 0 !important;">
                ...
                <tbody>
                    ...
                    try {
                            ...
                            <tr id="row-unit-<?php echo $as["id"]; ?>" data-id="<?php echo $as["id"]; ?>" data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="definition-tag-dot mr-2" style="background: #7c3aed;"></div>
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="user-avatar-mini mr-2" style="width: 28px; height: 28px; border-radius: 50%; background: #ede9fe; color: #6d28d9; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">
                                ...
                                <td class="text-center" style="vertical-align: middle; white-space: nowrap;">
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        <button type="button" class="btn btn-outline-danger btn-delete-unit" 
                                            ...
                                            data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                            title="Sil" 
                                            onClick="deleteRecord('\'<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>\' servis durumunu silmek istediğinize emin misiniz?','<?php echo $as["id"]; ?>','service-status','units')">
                        ...
                        echo '<tr><td colspan="6" class="text-center text-danger py-4"><i class="fa fa-exclamation-triangle mr-2"></i> Servis durumları yüklenirken bir hata oluştu: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . '</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
...
<!-- Servis Durumu Ekleme / Düzenleme Modalı -->
        ...
        <div class="modal-content premium-modal-content">
            <form method="POST" id="myForm" onsubmit="return false;">
                ...
                <div class="modal-header premium-modal-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <div class="modal-header-icon mr-3" style="background: linear-gradient(135deg, #7c3aed 0%, #6366f1 100%);">
                            <i class="fa fa-tasks" id="modalHeaderIcon"></i>
                        </div>
                        <div>
                            <h5 class="modal-title font-weight-bold mb-0" id="exampleModalLongTitle">Yeni Servis Durumu</h5>
                            <small class="text-muted" id="modalSubtitle">Sistemde kullanılacak yeni bir servis durumu tanımlayın</small>
                        </div>
                    </div>
                ...
                <div class="modal-body p-4">
                    <div class="form-group mb-3">
                        <label for="title" class="font-weight-600 text-dark mb-1">
                            Servis Durumu Adı <span class="text-danger">*</span>
                        </label>
                        <div class="input-group input-group-modern">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><i class="fa fa-tasks text-muted"></i></span>
                            </div>
                            <input required type="text" class="form-control form-control-modern" name="title" id="title" placeholder="Örn: Parça Bekliyor" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div class="suggestion-box p-3 rounded" style="background: #f8fafc; border: 1px dashed #cbd5e1;">
                        ...
                        <div class="suggestion-tags d-flex flex-wrap" style="gap: 8px;">
                            <span class="badge badge-pill quick-tag" role="button" data-val="Açık"><i class="fa fa-tag mr-1 text-muted"></i> Açık</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Beklemede"><i class="fa fa-tag mr-1 text-muted"></i> Beklemede</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Parça Bekliyor"><i class="fa fa-tag mr-1 text-muted"></i> Parça Bekliyor</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Teknisyen Atandı"><i class="fa fa-tag mr-1 text-muted"></i> Teknisyen Atandı</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Test Aşamasında"><i class="fa fa-tag mr-1 text-muted"></i> Test Aşamasında</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Tamamlandı"><i class="fa fa-tag mr-1 text-muted"></i> Tamamlandı</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="İptal Edildi"><i class="fa fa-tag mr-1 text-muted"></i> İptal Edildi</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer premium-modal-footer d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" id="submitButtonByAjax" class="btn btn-primary btn-modern-save" style="background: linear-gradient(135deg, #7c3aed 0%, #6366f1 100%);">
...
.quick-tag:hover {
    ...
    box-shadow: 0 3px 6px rgba(124, 58, 237, 0.15) !important;
}
.quick-tag:hover i {
...
$(document).ready(function() {
    var pageSlug = "service-status";
    var entityLabel = "Servis Durumu";
    ...
    // ==========================================
    $(document).on("contextmenu", "#serviceStatusTable tbody tr", function(e) {
        if ($(this).find("td").length <= 1) return;

        e.preventDefault();

        var $tr = $(this);
        $("#serviceStatusTable tbody tr").removeClass("context-menu-active");
        ...
        var menuHtml = '<div class="cm-header"><i class="fa fa-tasks mr-1 text-primary"></i> ' + $("<div>").text(title || entityLabel).html() + '</div>';
