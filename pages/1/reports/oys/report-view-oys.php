...
if (empty($_GET['send-mail']) || $_GET['send-mail'] !== 'true') {
    ...
    send_pdf_email_attachment(
        $pdf_content,
        $document . '.pdf',
        'index.php?p=report-send-as-mail&id=' . urlencode((string)$id) . '&type=oys&st=success-mail',
        'index.php?p=report-send-as-mail&id=' . urlencode((string)$id) . '&type=oys&st=unsuccessful',
        'Yangın Otomatik Söndürme Kontrol Raporu - ' . ($report['report_number'] ?? ''),
        'Yangın otomatik söndürme kontrol raporunuz ekte sunulmuştur.'
    );
}
