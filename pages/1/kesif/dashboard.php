...
.dark-mode .kd-hero,.dark-mode .kd-card,.dark-mode .kd-kpi,.dark-mode .kd-filter{background:#1e293b;border-color:#334155}.dark-mode .kd-title,.dark-mode .kd-card-title,.dark-mode .kd-kpi-value,.dark-mode .kd-rank-name{color:#f1f5f9}.dark-mode .kd-btn,.dark-mode .kd-pill,.dark-mode .kd-date-form input{background:#0f172a;border-color:#475569;color:#cbd5e1}.dark-mode .kd-card-head,.dark-mode .kd-rank,.dark-mode .kd-table td{border-color:#334155}.dark-mode .kd-table th{background:#0f172a;color:#94a3b8}.dark-mode .kd-table{color:#cbd5e1}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new ApexCharts(document.querySelector('#kesifTrendChart'), {chart:{type:'area',height:270,toolbar:{show:false},fontFamily:'inherit'},series:[{name:'Keşif',data:<?php echo json_encode($monthValues); ?>}],xaxis:{categories:<?php echo json_encode($monthLabels, JSON_UNESCAPED_UNICODE); ?>,labels:{style:{colors:textColor,fontSize:'10px'}}},yaxis:{min:0,forceNiceScale:true,labels:{style:{colors:textColor},formatter:function(v){return Math.round(v)}}},colors:['#0284c7'],stroke:{curve:'smooth',width:2.5},fill:{type:'gradient',gradient:{shadeIntensity:1,opacityFrom:.28,opacityTo:.03}},dataLabels:{enabled:false},grid:{borderColor:dark?'#334155':'#eef2f7'},tooltip:{theme:dark?'dark':'light'}}).render();
    new ApexCharts(document.querySelector('#kesifStatusChart'), {chart:{type:'donut',height:270,fontFamily:'inherit'},series:<?php echo json_encode($statusChartValues); ?>,labels:<?php echo json_encode($statusChartLabels, JSON_UNESCAPED_UNICODE); ?>,colors:<?php echo json_encode($statusChartColors); ?>,legend:{position:'bottom',fontSize:'10px',labels:{colors:textColor}},dataLabels:{enabled:true,formatter:function(v){return Math.round(v)+'%'}},plotOptions:{pie:{donut:{size:'64%',labels:{show:true,total:{show:true,label:'Toplam',formatter:function(){return '<?php echo $total; ?>'}}}}}},noData:{text:'Veri bulunamadı'}}).render();
});
</script>
