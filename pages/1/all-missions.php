...
if (isset($_GET["id"]) && @$_GET["mode"] === "delete" && (permtrue("allmisview") || permtrue("missionadd"))) {
    ...
    if ($deleted) {
        ...
        header("Location: index.php?p=all-missions&st=error");
    }
    exit;
}

// Durum güncelleme (Yapıldı işaretle)
if (@$_GET["mode"] === "update" && isset($_GET["mid"])) {
    $mid = (int)$_GET["mid"];
    $newStatu = isset($_GET["statu"]) ? (int)$_GET["statu"] : 1;
    $missionModel->updateStatus($mid, $newStatu, $currentUserId);
    header("Location: index.php?p=all-missions&update=true&mid=" . $mid);
    exit;
}

// Görev listesi ve istatistikler
$allMissions = $missionModel->getAllMissions();
$stats = $missionModel->getMissionStats($allMissions);
...
<script>
    (function() {
        try {
            if (localStorage.getItem('aydinogullari_kpi_allmissions_collapsed') === 'true') {
                document.documentElement.classList.add('kpi-allmissions-collapsed-early');
            }
        } catch(e) {}
    })();
</script>
...
<div class="mission-module-wrapper">
    ...
    <div class="premium-header-card animate-fade-in mb-3">
        <div class="header-content">
            <div class="header-left">
                <div class="header-icon mission-header-icon" style="background: linear-gradient(135deg, #0284c7, #0369a1) !important;">
                    <i class="fa fa-cubes"></i>
                </div>
                <div class="header-title">
                    <h4>Sistemdeki Tüm Görevler</h4>
                    <div class="mission-stat-pills">
                        ...
                        <?php if ($stats['overdue'] > 0): ?>
                            <span class="mission-stat-pill mission-stat-overdue">
                                <i class="fa fa-exclamation-triangle"></i> Geciken: <strong><?php echo $stats['overdue']; ?></strong>
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div id="kpiSummarySection" class="mission-kpi-grid mb-3 animate-fade-in kpi-summary-collapse">
        <div class="crm-kpi-card">
            <div class="crm-kpi-header">
                <div>
                    <span class="crm-kpi-label">Toplam Görev</span>
        ...
        <div class="crm-kpi-card">
            <div class="crm-kpi-header">
                <div>
                    <span class="crm-kpi-label">Açık / Bekleyen</span>
        ...
        <div class="crm-kpi-card">
            <div class="crm-kpi-header">
                <div>
                    <span class="crm-kpi-label">Yüksek Aciliyetli</span>
    ...
    <?php if (@$_GET["send-mail"] === "true"): ?>
        <div class="alert alert-success alert-dismissible fade show animate-fade-in mb-3" role="alert" style="border-radius: 10px;">
            <i class="fa fa-envelope mr-2"></i> Görev oluşturuldu ve ilgili personele bilgilendirme maili gönderildi.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Kapat"></button>
        </div>
    <?php endif; ?>
    ...
    <?php if (@$_GET["update"] === "true"): ?>
        <div class="alert alert-success alert-dismissible fade show animate-fade-in mb-3" role="alert" style="border-radius: 10px;">
            <i class="fa fa-check-circle mr-2"></i> Görev başarıyla tamamlandı olarak işaretlendi.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Kapat"></button>
        </div>
    <?php endif; ?>
    ...
    <?php if (@$_GET["deleted"] === "true"): ?>
        <div class="alert alert-success alert-dismissible fade show animate-fade-in mb-3" role="alert" style="border-radius: 10px;">
            <i class="fa fa-check-circle mr-2"></i> Görev sistemden silindi.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Kapat"></button>
        </div>
    <?php endif; ?>
    ...
    <div class="form-card mb-4 animate-fade-in mission-table-card">
        <div class="form-card-header mission-table-header">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h5 class="mb-0">Tüm Görevler Listesi</h5>
                    <p class="mb-0">Sistemde oluşturulmuş tüm görevlerin atama, durum ve süreçlerini genel olarak izleyin.</p>
                </div>
            </div>
            <div class="d-flex align-items-center" style="gap: 8px;">
                <div id="allMissionsSearchContainer" class="mission-search-container"></div>
        ...
        <div class="table-responsive mission-table-responsive">
            <table id="allMissionsTable" class="data-table select-row table-hover table-bordered premium-table">
                <thead>
                    <tr>
                        ...
                        <th data-filter-type="text" style="width: 150px;">Oluşturan</th>
                        <th data-filter-type="text" style="width: 190px;">Görevi Yapacak Kişi(ler)</th>
                        <th data-filter-type="date" style="width: 110px;">Başlangıç</th>
                        <th data-filter-type="date" style="width: 130px;">Son Tarih</th>
                            ...
                            <td colspan="9" class="text-center py-5 text-muted">
                                <div class="empty-state-wrap">
                                    <div class="empty-icon mb-3">
                                        <i class="fa fa-folder-open-o" style="font-size: 38px; color: #cbd5e1;"></i>
                                    </div>
                                    <h6 style="color: #64748b; font-weight: 600;">Sistemde Kayıtlı Görev Bulunmuyor</h6>
                                    <p class="small text-muted mb-0">İlk görevi oluşturmak için "Yeni Görev Oluştur" butonuna tıklayabilirsiniz.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        ?>
                            <tr class="<?php echo $isDone ? 'mission-row-done' : ($isOverdue ? 'mission-row-overdue' : ''); ?>">
                                ...
                                <!-- Oluşturan -->
                                <td>
                                    <div class="mission-user-badge">
                                        <div class="mission-avatar"><?php echo $creatorInitial; ?></div>
                                        <div class="mission-user-text">
                                            <span class="mission-username"><?php echo $creatorName; ?></span>
                                            <?php if (!empty($creatorTitle)): ?>
                                                <small class="mission-usertitle"><?php echo $creatorTitle; ?></small>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </td>
                                    ...
                                    <div class="mission-action-buttons">
                                        ...
                                        <?php if (permtrue("allmisview") || permtrue("missionadd")): ?>
                                            <button type="button" 
                                                    ...
                                                    title="Görevi Sil" 
                                                    onclick="confirmDeleteAllMission(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars(addslashes($row['title'])); ?>')">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content premium-modal-content">
            ...
            <div class="modal-body p-4">
                ...
                <div class="preview-section mb-3" id="previewAuthorsSection">
...
<style>
/* Flicker Prevention */
.kpi-allmissions-collapsed-early #kpiSummarySection {
...
<script>
function confirmDeleteAllMission(missionId, missionTitle) {
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            title: 'Görevi Sil',
            html: '<b>"' + missionTitle + '"</b> başlıklı görevi sistemden tamamen silmek istediğinize emin misiniz?<br><small class="text-muted">Bu işlem geri alınamaz.</small>',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                window.location.href = 'index.php?p=all-missions&mode=delete&id=' + missionId;
            }
        });
    } else {
...
$(document).ready(function () {
    ...
    var KPI_STORAGE_KEY = 'aydinogullari_kpi_allmissions_collapsed';
    ...
    function initAllMissionsTable() {
        $("#allMissionsTable").find("tr.search-input-row").remove();

        var filterEl = $("#allMissionsTable_wrapper .dataTables_filter");
        if (filterEl.length && !$("#allMissionsSearchContainer .dataTables_filter").length) {
            filterEl.appendTo("#allMissionsSearchContainer");
            if (filterEl[0]) filterEl[0].dataset.relocated = 'true';
        }

        if (window.App && window.App.TableFilter) {
            App.TableFilter.attachToTable(document.getElementById("allMissionsTable"));
        }
    }
    ...
    initAllMissionsTable();
    setTimeout(initAllMissionsTable, 100);
    setTimeout(initAllMissionsTable, 300);
});
</script>