...
if (@$_GET["id"] && @$_GET["mode"] == "delete" && @$_GET["code"] == "04md177") {
    ...
    if ($contq->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($deletq) {
            header("Location: index.php?p=view-outdocument&id=$cdid&type=delete");
            exit;
        }
    }
}
...
$waitingReceiveCount = (int)($stats['waiting_receive_count'] ?? 0);
$receivedCount = (int)($stats['received_count'] ?? 0);
$completedCount = (int)($stats['completed_count'] ?? 0);
$receiveRate = $totalCount > 0 ? round(($receivedCount / $totalCount) * 100, 1) : 0;

try {
    $logger = \getLogger("Evrak Takip");
    $logger->info("Giden evrak listesi görüntülendi.", [
...
<script>
    (function() {
        try {
            if (localStorage.getItem('aydinogullari_kpi_outdoc_collapsed') === 'true') {
                document.documentElement.classList.add('kpi-outdoc-collapsed-early');
            }
        } catch(e) {}
    })();
</script>
...
<style>
    /* ==========================================
       PREMIUM OUT-DOCUMENTS LIST THEME
       ========================================== */
    .kpi-outdoc-collapsed-early #kpiSummarySection {
    ...
    .soft-teal    { background: #ccfbf1; color: #115e59; }
    ...
    #tblOutDocuments thead th.sorting,
    #tblOutDocuments thead th.sorting_asc,
    #tblOutDocuments thead th.sorting_desc {
        padding-left: 28px !important;
        padding-right: 28px !important;
    }
    #tblOutDocuments thead th:not(.sorting):not(.sorting_asc):not(.sorting_desc) {
        padding-left: 10px !important;
        padding-right: 10px !important;
    }
    #tblOutDocuments tbody td {
        padding: 10px 10px;
        vertical-align: middle;
        font-size: 13px;
        color: #334155;
    ...
    #tblOutDocuments tbody tr:hover {
        background-color: #f8fafc;
    }
    #tblOutDocuments tbody tr.context-menu-active {
    ...
    .dark-mode .icon-teal    { background: rgba(13, 148, 136, 0.15) !important; color: #2dd4bf !important; }
    ...
    .dark-mode .soft-teal    { background: rgba(13, 148, 136, 0.2) !important; color: #5eead4 !important; }
    ...
    .dark-mode #tblOutDocuments thead th {
        background: #0f172a !important;
        color: #94a3b8 !important;
        border-bottom-color: #334155 !important;
    }
    .dark-mode #tblOutDocuments tbody td {
        border-top-color: #334155 !important;
        color: #e2e8f0 !important;
    }
    .dark-mode #tblOutDocuments tbody tr:hover {
        background-color: rgba(255, 255, 255, 0.03) !important;
    }
    .dark-mode .badge-doc-type-out {
...
<div class="outdoc-list-page-container">
    <div class="outdoc-list-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap" style="gap: 12px; padding: 0;">
            <div class="page-title-box">
                ...
                <div class="page-title-text">
                    <h4>Giden Evrak Takibi</h4>
                    <p>Sistemden gönderilen tüm evraklar, teslimat ve onay durumları</p>
                </div>
            </div>
            <div class="d-flex align-items-center flex-wrap" style="gap: 8px;">
                <button type="button" class="btn btn-outline-secondary btn-action-outline" id="btnRefreshOutDocs" title="Tabloyu Yenile">
        ...
        <div id="kpiSummarySection" class="row mx-0 mb-2 kpi-summary-collapse">
            ...
            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 mb-3 mb-xl-0 px-1">
                <div class="crm-kpi-card">
                    <div class="crm-kpi-header">
                        <div>
                            <span class="crm-kpi-label">Toplam Giden Evrak</span>
                            <div class="crm-kpi-value"><?php echo number_format($totalCount, 0, ',', '.'); ?></div>
                        </div>
                        <div class="crm-kpi-icon icon-teal">
                            <i class="fa fa-send-o"></i>
                        </div>
                    </div>
                    <div class="crm-kpi-footer">
                        <span class="text-muted font-11">Kayıtlı Giden Evrak</span>
                        <span class="crm-badge-soft soft-teal">Giden</span>
                    </div>
                </div>
            </div>

            <!-- Teslim Alınmayı Bekleyenler -->
            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 mb-3 mb-xl-0 px-1">
                <div class="crm-kpi-card">
                    <div class="crm-kpi-header">
                        <div>
                            <span class="crm-kpi-label">Teslim Bekleyenler</span>
                            <div class="crm-kpi-value"><?php echo number_format($waitingReceiveCount, 0, ',', '.'); ?></div>
                        </div>
                    ...
                    <div class="crm-kpi-footer">
                        <span class="text-muted font-11">Kabul Bekleyen Evrak</span>
            ...
            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 mb-3 mb-xl-0 px-1">
                <div class="crm-kpi-card">
                    <div class="crm-kpi-header">
                        <div>
                            <span class="crm-kpi-label">Teslim Alınanlar</span>
                            <div class="crm-kpi-value"><?php echo number_format($receivedCount, 0, ',', '.'); ?></div>
                        </div>
                    ...
                    <div class="crm-kpi-footer">
                        <span class="text-muted font-11">%<?php echo $receiveRate; ?> Teslim Oranı</span>
                        <span class="crm-badge-soft soft-cyan">Teslim Edildi</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12 mb-3 mb-xl-0 px-1">
                <div class="crm-kpi-card">
                    ...
                    <div class="crm-kpi-footer">
                        <span class="text-muted font-11">Süreç Kapatılanlar</span>
        ...
        <div class="form-card animate-fade-in">
            ...
            <div class="table-responsive">
                <table id="tblOutDocuments" class="data-table table table-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 130px;" class="text-center">Teslim Alan</th>
                    ...
                    <tbody>
                        ...
                        while ($as = $cq->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $evrakTuru = htmlspecialchars($as["evrakturu"] ?? 'Giden');
                            $kategori = htmlspecialchars($as["kategori"] ?? '-');
                            $adet = htmlspecialchars($as["adet"] ?? '1');
                            $teslimAlanId = (int)($as["teslimalan"] ?? 0);
                            $teslimAlanName = htmlspecialchars($as["teslim_alan_username"] ?? '');
                            $teslimAlmaTarihi = $as["teslimalmatarihi"] ?? '';
                        ...
                        <tr data-doc-id="<?php echo $docId; ?>" data-company="<?php echo $companyName; ?>">
                            ...
                            <td>
                                <span class="badge-doc-type-out"><?php echo $evrakTuru; ?></span>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <?php
                                if ($teslimAlanId == $currentLid && ($teslimAlmaTarihi == "0000-00-00 00:00:00" || empty($teslimAlmaTarihi))) {
                                    echo '<button type="button" class="btn btn-sm btn-success receive" data-id="' . $docId . '" style="border-radius: 6px; padding: 3px 10px; font-size: 11.5px; font-weight: 600;"><i class="fa fa-check mr-1"></i>Teslim Al</button>';
                                } elseif (!empty($teslimAlmaTarihi) && $teslimAlmaTarihi != "0000-00-00 00:00:00") {
                                    echo '<span class="font-12 text-dark font-weight-500"><i class="fa fa-user-check text-success mr-1"></i>' . $teslimAlanName . '</span>';
                                } else {
                                    echo '<span class="badge badge-light text-muted font-11">Teslim Bekliyor</span>';
                                }
                                ?>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="action-btn-group">
                                    ...
                                    <a href="#" class="btn btn-sm btn-outline-danger action-btn" data-tooltip="Sil" onClick="deleteRecord('<?php echo addslashes($companyName); ?> firmasına ait evrak kaydını silmek istediğinize emin misiniz?', '<?php echo $docId; ?>', 'view-outdocument', 'evraktakip')">
...
<script>
    $(document).ready(function () {
        ...
        var KPI_STORAGE_KEY = 'aydinogullari_kpi_outdoc_collapsed';
        ...
        // DataTables Kurulumu
        var outDocTable = $('#tblOutDocuments').DataTable({
            responsive: false,
            autoWidth: false,
            scrollX: false,
            pageLength: 25,
            ...
            initComplete: function () {
                ...
                var $searchBox = $('#tblOutDocuments_filter');
        ...
        // Yenile Butonu
        $('#btnRefreshOutDocs').on('click', function() {
        ...
        $('#btnExportOutDocs').on('click', function () {
            ...
            var rows = [];
            rows.push(['Sıra', 'Firma', 'Evrak Türü', 'Kategori', 'Adet', 'Teslim Alan', 'Teslim Eden', 'Teslim Tarihi', 'Evrak Durumu', 'Açıklama']);

            outDocTable.rows({ search: 'applied' }).every(function() {
                var $row = $(this.node());
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Giden Evrak Listesi');
            XLSX.writeFile(wb, 'Giden_Evrak_Listesi_' + new Date().toISOString().slice(0, 10) + '.xlsx');
        });

        // Teslim Al Butonu AJAX Mantığı
        ...
        $(document).on('contextmenu', '#tblOutDocuments tbody tr', function(e) {
            if ($(this).find('td').length <= 1) return;

            e.preventDefault();
            
            var $tr = $(this);
            $('#tblOutDocuments tbody tr').removeClass('context-menu-active');
            ...
            var $receiveBtn = $tr.find('.receive');
            
            var menuHtml = '<div class="cm-header"><i class="fa fa-paper-plane-o mr-1"></i> ' + $('<div>').text(companyName).html() + '</div>';

            // 0. Teslim Al butonu varsa
            if ($receiveBtn.length) {
                var docId = $receiveBtn.attr('data-id');
                menuHtml += '<a href="#" class="receive" data-id="' + docId + '"><i class="fa fa-check text-success mr-2"></i> Evrağı Teslim Al</a>';
