...
$statuCode = 2; // Servis Konusu
$pageSlug = "service-type";
$pageTitle = "Servis Konusu Tanımlama";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Lütfen servis konusu başlığını giriniz.']);
            exit;
        }
    } else {
        if ($type === "new" || $id === 0) {
            try {
                ...
                $ekle->execute([$title, $regdate, $statuCode, $creator, "Servis Konusu Tanımı"]);
                $lastId = $ac->lastInsertId();

                if (function_exists('audit_log')) {
                    audit_log("create", $pageSlug, "Yeni servis konusu eklendi: " . $title, "units", (string)$lastId);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Servis konusu başarıyla eklendi.', 'id' => $lastId]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Service type insert error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
            ...
            try {
                ...
                if (function_exists('audit_log')) {
                    audit_log("update", $pageSlug, "Servis konusu güncellendi: " . $title, "units", (string)$id);
                }

                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'success', 'message' => 'Servis konusu başarıyla güncellendi.', 'id' => $id]);
                    exit;
                }
            } catch (PDOException $e) {
                error_log("Service type update error: " . $e->getMessage());
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
                    header('Content-Type: application/json', true, 500);
...
try {
    ...
    error_log("Service type stats error: " . $e->getMessage());
}
?>
    ...
    <div class="premium-header-card mb-4">
        <div class="header-content">
            <div class="header-left">
                ...
                <div class="header-title">
                    ...
                    <div class="header-stat-pills d-flex align-items-center flex-wrap" style="gap: 8px; margin-top: 6px;">
                        <span class="header-pill header-pill-total" title="Toplam Kayıt">
                            <i class="fa fa-list-ul mr-1"></i> Toplam: <strong><?php echo $totalCount; ?></strong>
                        </span>
                        <?php if ($lastAddedTitle !== "-"): ?>
                        <span class="header-pill header-pill-recent" title="Son Eklenen Servis Konusu">
            ...
            <div class="header-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <button type="button" class="btn-header btn-header-save" id="btnOpenNewModal">
                    <i class="fa fa-plus-circle mr-1"></i> Yeni Servis Konusu
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="form-card mb-4 animate-fade-in" style="padding: 0; overflow: hidden; border-radius: 14px;">
        <div class="form-card-header d-flex justify-content-between align-items-center flex-wrap" style="padding: 16px 20px; border-bottom: 1px solid #f1f5f9; gap: 15px;">
            <div class="d-flex align-items-center header-left-inner">
                <div class="card-icon mr-3" style="width: 40px; height: 40px; border-radius: 10px; font-size: 16px; display: flex; align-items: center; justify-content: center; background: rgba(79, 70, 229, 0.1); color: #4f46e5;">
                    <i class="fa fa-th-list"></i>
                </div>
                <div>
                    <h5 class="mb-0" style="font-size: 16px; font-weight: 700;">Tanımlı Servis Konuları</h5>
                    <p class="mb-0 text-muted" style="font-size: 12.5px;">Servis ve projelerde seçilebilecek servis konularını yönetin (Sağ tık menüsü desteklenir)</p>
                </div>
            </div>
        </div>

        <div class="table-responsive" style="padding: 0; margin: 0;">
            <table id="serviceTypeTable" class="data-table select-row table-bordered table-hover" style="width: 100%; margin: 0 !important;">
                <thead>
                    <tr>
                        <th class="text-center no-filter" style="width: 60px; max-width: 60px;">#Sıra</th>
                        <th style="min-width: 240px;">Servis Konusu</th>
                ...
                <tbody>
                    ...
                    try {
                            ...
                            <tr id="row-unit-<?php echo $as["id"]; ?>" data-id="<?php echo $as["id"]; ?>" data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="definition-tag-dot mr-2"></div>
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="d-flex align-items-center">
                                        <div class="user-avatar-mini mr-2" style="width: 28px; height: 28px; border-radius: 50%; background: #e0e7ff; color: #4338ca; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">
                                ...
                                <td class="text-center" style="vertical-align: middle; white-space: nowrap;">
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        <button type="button" class="btn btn-outline-danger btn-delete-unit" 
                                            ...
                                            data-title="<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                            title="Sil" 
                                            onClick="deleteRecord('\'<?php echo htmlspecialchars($as["title"] ?? '', ENT_QUOTES, 'UTF-8'); ?>\' servis konusunu silmek istediğinize emin misiniz?','<?php echo $as["id"]; ?>','service-type','units')">
                        ...
                        echo '<tr><td colspan="6" class="text-center text-danger py-4"><i class="fa fa-exclamation-triangle mr-2"></i> Servis konuları yüklenirken bir hata oluştu: ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . '</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
...
<!-- Servis Konusu Ekleme / Düzenleme Modalı -->
                ...
                <div class="modal-header premium-modal-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <div class="modal-header-icon mr-3" style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%);">
                            <i class="fa fa-wrench" id="modalHeaderIcon"></i>
                        </div>
                        <div>
                            <h5 class="modal-title font-weight-bold mb-0" id="exampleModalLongTitle">Yeni Servis Konusu</h5>
                            <small class="text-muted" id="modalSubtitle">Sistemde kullanılacak yeni bir servis konusu tanımlayın</small>
                        </div>
                    </div>
                ...
                <div class="modal-body p-4">
                    <div class="form-group mb-3">
                        <label for="title" class="font-weight-600 text-dark mb-1">
                            Servis Konusu Adı <span class="text-danger">*</span>
                        </label>
                        <div class="input-group input-group-modern">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><i class="fa fa-wrench text-muted"></i></span>
                            </div>
                            <input required type="text" class="form-control form-control-modern" name="title" id="title" placeholder="Örn: Periyodik Bakım" autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div class="suggestion-box p-3 rounded" style="background: #f8fafc; border: 1px dashed #cbd5e1;">
                        ...
                        <div class="suggestion-tags d-flex flex-wrap" style="gap: 8px;">
                            <span class="badge badge-pill quick-tag" role="button" data-val="Periyodik Bakım"><i class="fa fa-tag mr-1 text-muted"></i> Periyodik Bakım</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Arıza Onarım"><i class="fa fa-tag mr-1 text-muted"></i> Arıza Onarım</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Sistem Kurulumu"><i class="fa fa-tag mr-1 text-muted"></i> Sistem Kurulumu</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Mekanik Kurulum"><i class="fa fa-tag mr-1 text-muted"></i> Mekanik Kurulum</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Algılama Kurulum"><i class="fa fa-tag mr-1 text-muted"></i> Algılama Kurulum</span>
                            <span class="badge badge-pill quick-tag" role="button" data-val="Periyodik Kontrol ve Raporlama"><i class="fa fa-tag mr-1 text-muted"></i> Periyodik Kontrol ve Raporlama</span>
                        </div>
                    </div>
                </div>
...
.header-pill-total strong { color: #3730a3; }
.header-pill-recent {
    background: #f0fdf4;
    border-color: #bbf7d0;
    color: #15803d;
...
.quick-tag:hover {
    ...
    box-shadow: 0 3px 6px rgba(79, 70, 229, 0.15) !important;
}
.quick-tag:hover i {
...
$(document).ready(function() {
    var pageSlug = "service-type";
    var entityLabel = "Servis Konusu";
    ...
    // ==========================================
    $(document).on("contextmenu", "#serviceTypeTable tbody tr", function(e) {
        if ($(this).find("td").length <= 1) return;

        e.preventDefault();

        var $tr = $(this);
        $("#serviceTypeTable tbody tr").removeClass("context-menu-active");
        ...
        var menuHtml = '<div class="cm-header"><i class="fa fa-wrench mr-1 text-primary"></i> ' + $("<div>").text(title || entityLabel).html() + '</div>';
