...
<div class="content pd-20 bg-white border-radius-16 box-shadow mb-30">
	...
	<div class="clearfix mb-20">
		...
		<?php if (permtrue("uadd")) { ?>
			<a href="index.php?p=new-user"><button type="button" class="btn btn-primary btn-sm float-right"><i class="fa fa-plus"></i> Yeni Kullanıcı </button></a>
		<?php } ?><br><br>
	</div>
	<table class="data-table table-hover table-bordered" style="text-align: center;">
		<thead>
			<tr>
		...
		<tbody>
			...
			?>
				<tr>
					...
					<td>
						<?php  
						if ($as["statu"] == 0) { echo '<span class="btn btn-secondary btn-sm"><i class="fa fa-close"></i> Pasif</span>'; } 
						else { echo '<span class="btn btn-success btn-sm"><i class="fa fa-check"></i> Aktif</span>'; } 	
							?>	
				
					</td>
					<td>
						<?php if (permtrue("uedit")) { ?>
							<a href="index.php?p=edit-user&id=<?php echo $as["id"]; ?>" class="btn btn-sm btn-outline-info" data-tooltip="Düzenle"><i class="fa fa-pencil"></i></a>
							<?php
							if ($as["statu"] == 1 && $as["permission"] != 1) {
							?>
							<a class="btn btn-sm btn-secondary text-white"  data-tooltip="Pasifleştir" href="index.php?p=all-users&mode=updatest&code=3222891&reg=true&md=active&id=<?php echo $as["id"]; ?>&stu=0"><i class="fa fa-user-times"></i></a>
							<?php
							} elseif ($as["statu"] == 0 && $as["permission"] != 1) {
							?>
								<a  class="btn btn-sm btn-success text-white" data-tooltip="Aktifleştir" href="index.php?p=all-users&mode=updatest&code=3222891&reg=true&md=active&id=<?php echo $as["id"]; ?>&stu=1"><i class="fa fa-user-circle"></i></a>
							<?php
							}
						}
						if (permtrue("udelete") and $as["id"] != sesset("id") and $as["id"] != 1) { ?>
							<a class="btn btn-sm btn-danger text-white"  data-tooltip="Sil"  onClick="deleteRecord('Devam ettiğiniz takdirde,kullanıcıya ait tüm bilgiler ve kullanıcı adına düzenlenmiş olan teklif & projeler tamamen silinecektir. Devam etmek istiyor musunuz?','<?php echo $as['id']; ?>','all-users')" >
							<i class="fa fa-trash"></i></a>
							<?php } ?>

							<a class="btn btn-sm btn-warning text-black" data-tooltip="Pozisyon" href="index.php?p=edit-perm&reg=true&md=update&id=<?php echo $ppa["id"]; ?>"><i class="fa fa-cog"></i></a>
